...
export default function NavBar({ title, items = [] }: NavBarProps) {
  ...
  return (
    <nav className={`flex flex-col ${showMenu ? 'h-full' : 'h-auto'} uppercase md:justify-between md:h-auto border-b border-white/20 lg:flex-row`}>
      ...
      <div className={`grid grid-cols-1 content-start h-full pr-8 pl-6 divide-y ${showMenu ? 'block' : 'hidden'} md:divide-y-0 divide-white/20 md:flex md:gap-x-8 md:justify-around md:h-auto md:visible md:pt-0 md:my-6 lg:my-0`}>
